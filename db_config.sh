...
mysql -uroot -pqwe123@ -e "CREATE DATABASE IF NOT EXISTS dbname;" >> /tmp/vm_build.log 2>&1
mysql -uroot -pqwe123@ -e "show databases;"

# Create a new user
echo -e "\n--=== Create MySQL User ===--\n"
mysql -uroot -pqwe123@ -e "grant all privileges on dbname.* to 'dbuser'@'%' identified by 'qwe123@'" > /tmp/vm_build.log 2>&1
